...
function list() {
    ...
    logger.debug('PaymentInstruments.list()');

    var isEnableGmoController = int_gmo.getController('GMO_PAYMENT');
    var isEnableGmo           = isEnableGmoController.IsEnableGMO();
    var viewObj;
    if(isEnableGmo.isEnableCreditCard){
        logger.debug('enable gmo');
        // getCartInfo
        var getCartInfoController = int_gmo.getController('GMO_PAYMENT');
        viewObj = getCartInfoController.GetCartInfo();
    }else{
    ...
    app.getView(viewObj).render('account/payment/paymentinstrumentlist');
}
...
function save(params) {
    var saveCustomerCreditCard = require('app_gmo_core/cartridge/scripts/checkout/SaveCustomerCreditCard');
...
function create() {
    ...
    //IsEnableGMO
    var isEnableGmoController = int_gmo.getController('GMO_PAYMENT');
    var gmoEnableconfig       = isEnableGmoController.IsEnableGMO();
    if(gmoEnableconfig.isEnableCreditCard){
    	paymentInstrument.creditCardType = newCreditCardForm.get('type').value();
        logger.debug(' paymentInstrument.creditCardType='+paymentInstrument.creditCardType);
        var addCardController = int_gmo.getController('GMO_PAYMENT');
        var cardRegistarStatus = addCardController.AddCard(paymentInstrument, null, null, null);
        logger.debug('Add Card Status --> {0}', JSON.stringify(cardRegistarStatus));
...
/**
 * Card registration at GMO-PG.
 * @transaction
 * @return {boolean} true if the credit card can be verified, false otherwise
 */
function autoRegistrationCard(paymentInstrument) {

    // CurrentForms.billing
    var billingForm  = app.getForm('billing');
    // CurrentForms.paymentinstruments
    var paymentForm = app.getForm('paymentinstruments');
    var newCreditCardForm = paymentForm.get('creditcards.newcreditcard');

    // Assign
    newCreditCardForm.setValue('number', billingForm.getValue('paymentMethods.creditCard.number'));
    newCreditCardForm.setValue('cvn', billingForm.getValue('paymentMethods.creditCard.cvn'));
    newCreditCardForm.setValue('expiration.month', billingForm.getValue('paymentMethods.creditCard.expiration.month'));
    newCreditCardForm.setValue('expiration.year', billingForm.getValue('paymentMethods.creditCard.expiration.year'));
    newCreditCardForm.setValue('type', billingForm.getValue('paymentMethods.creditCard.type'));

    Transaction.begin();

    var addCardController = int_gmo.getController('GMO_PAYMENT');
    // addCard(CoSummaryCallBack, paymentInstrument, accessID, accessPass, Order)
    var cardRegistarStatus = addCardController.AddCard(paymentInstrument, null, null, null);
...
function Delete() {
    ...
    //---debug---
    var logger = Logger.getLogger('GMO');
    logger.debug('PaymentInstruments.Delete()');
    ...
    if(isEnableGmo.isEnableCreditCard){
        ...
        // deleteCardInfo
        var deleteCardInfoController = int_gmo.getController('GMO_PAYMENT');
        deleteCardInfoController.DeleteCardInfo();
...
function verifyCreditCard() {
    ...
    if (isEnableGmo.isEnableCreditCard) {
        // verifyCreditCard
        var verifyCreditCardController = int_gmo.getController('GMO_PAYMENT');
        var verifyCreditCard           = verifyCreditCardController.VerifyCreditCard();

        if (verifyCreditCard.ok === true) {
            return true;
        } else {
            return false;
        }
...
exports.AutoRegistrationCard = autoRegistrationCard;
